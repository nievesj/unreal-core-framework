...
#define BUI_TWO_PI          (6.28318530717f)
	...
	static float InOutExpo(float time, float duration = 1.0f)
	{
		...
		if (time == duration) return 1;
		time /= duration;
		if ((time * 0.5f) < 1) return 0.5f * (float)FMath::Pow(2, 10 * (time - 1));
		--time;
	...
	{
		time /= duration * 0.5f;
		if (time < 1) return -0.5f * ((float)FMath::Sqrt(1 - time * time) - 1);
		time -= 2;
	...
	static float InOutElastic(float time, float duration = 1.0f, float overshootOrAmplitude = 0.1f, float period = 1.0f)
	{
		...
		time -= 1;
		if (time < 1) return -0.5f * (overshootOrAmplitude * (float)FMath::Pow(2, 10 * time) * (float)FMath::Sin((time * duration - s) * BUI_TWO_PI / period));
